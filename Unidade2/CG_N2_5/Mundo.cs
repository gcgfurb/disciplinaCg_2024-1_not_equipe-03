...
namespace gcgcg
{
    public class Mundo : GameWindow
    {
        ...
        Ponto4D pontoCentral4D = new Ponto4D(0.3, 0.3);
        ...
        protected override void OnLoad()
        {
            ...
            this.pontoCentral = new Ponto(Mundo.mundo, ref this.rotuloAtual, new Ponto4D(0.3, 0.3));
            pontoCentral.PrimitivaTipo = PrimitiveType.Points;
            pontoCentral.PrimitivaTamanho = 10f;

            #region Objeto: circulo maior
            this.objetoSelecionado = new Circulo(mundo, ref rotuloAtual, 0.3, this.pontoCentral.PontosId(0));
            this.objetoSelecionado.shaderObjeto = new Shader("Shaders/shader.vert", "Shaders/shaderBranca.frag");
            #endregion

            #region Objeto: circulo menor
            this.circuloMenor = new Circulo(mundo, ref rotuloAtual, 0.1, this.pontoCentral.PontosId(0));
            this.circuloMenor.shaderObjeto = new Shader("Shaders/shader.vert", "Shaders/shaderBranca.frag");
            #endregion

            #region Objeto: retangulo
            double ptoRaioCirculo = Matematica.GerarPtosCirculoSimetrico(0.3);

            Ponto4D ptoSupDir = new Ponto4D(ptoRaioCirculo, ptoRaioCirculo) + this.pontoCentral.PontosId(0);
            Ponto4D ptoInfEsq = new Ponto4D(-ptoRaioCirculo, -ptoRaioCirculo) + this.pontoCentral.PontosId(0);

            this.retangulo = new Retangulo(Mundo.mundo, ref this.rotuloAtual, ptoInfEsq, ptoSupDir);
        ...
        protected override void OnUpdateFrame(FrameEventArgs e)
        {
            ...
            {
                this.TestarMovimento(this.pontoCentral.PontosId(0), 0.01, 0.0);
            } 
            ...
            {
                this.TestarMovimento(this.pontoCentral.PontosId(0), 0.0, 0.01);
            } 
        ...
        private void TestarMovimento(Ponto4D ponto, double x, double y) 
        {
            ponto.X += x;
            ponto.Y += y;

            if(Matematica.Dentro(this.retangulo.Bbox(), ponto)) 
            {
                this.retangulo.PrimitivaTipo = PrimitiveType.LineLoop;
                AtualizarPonto(ponto, x, y);

                return;
            }

            this.retangulo.PrimitivaTipo = PrimitiveType.Points;
            
            if (Matematica.distanciaQuadrado(ponto, new Ponto4D(0.3, 0.3)) <= Math.Pow(0.3, 2))
            {
                AtualizarPonto(ponto, x, y);
                return;
            }
        ...
        private void AtualizarPonto(Ponto4D novoPonto, double x, double y)  
        {
            this.pontoCentral.PontosAlterar(novoPonto, 0);
            this.pontoCentral.Atualizar();

            for (int i = 0; i < 72; i++)
            {
                this.circuloMenor.PontosAlterar(new Ponto4D(circuloMenor.PontosId(i).X + x, circuloMenor.PontosId(i).Y + y), i);
            }
            
            this.circuloMenor.ObjetoAtualizar();
        }
